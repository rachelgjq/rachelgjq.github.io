;(function($){
   $.fn.fsvs=function(options){
    ...
    var detecHash = function(){
    	$(options.selector,body).each(
    		function(i){
              var slide = $(this);
              if(!slide.attr('id')){
    ...
    var hasTransition = function(){
	    perfixes =['Webkit','Moz','ms','o'];
	    for(var i in perfixes){
	    	if(typeof document.getElementsByTagName('body')[0].style[perfixes[i]+'Transition']!== "undefined"){
	    		return true
	    	}
	    	return false;
	    }
    ...
    var bindMouseDrag = function(){
    	...
    	window.onmouseup = function(e){
    		if(e.y >(y + options.mouseSwipeDistance)){
    			app.slideUp();

    		}else if(e.y < (y - options.mouseSwipeDistance)){
    			app.slideDown();
    		}
    	...
    	$(window).on('touchmove',function(ev){
    		...
    		if(startY !== null){
    			...
    			if(touches && touches.length){
    				...
    				if(deltaY >= options.mouseSwipeDistance){
    					app.slideDown();
    					startY = null;
    				}
    				if(deltaY <= options.mouseSwipeDistance* -1){
    					app.slideUp();
    					startY = null;
    				}
    			}
